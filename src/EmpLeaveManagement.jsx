...
// //       this.setState({ teamLeaves: Array.isArray(res.data) ? res.data : [] });
// //     } catch (error) {
...
// //     console.log('approveLeave called', { recId, status, employeeId, currentUserId });
...
// //                       <td className="text-center1">{days} days</td>
// //                       <td>
...
// //                   Cancel
// //                 </button>
// //                 <button className="btn btn-danger" onClick={this.rejectLeave} disabled={!rejectReason.trim()}>
// //                   Confirm Rejection
// //                 </button>
// //               </div>
// //             </div>
...
export default class EmpLeaveManagement extends Component {
  ...
  loadMyLeaves = async () => {
    try {
      const res = await axios.get('/api/leave-record/myLeaves', { withCredentials: true });
      console.log("My Leaves Response:", res.data);
      this.setState({ myLeaves: Array.isArray(res.data) ? res.data : [] });
    } catch (error) {
      console.error('Error loading my leaves:', error);
    }
  ...
  loadTeamLeaves = async () => {
    try {
      const res = await axios.get('/api/leave-record/teamLeaves', { withCredentials: true });
      console.log("Team Leaves Full Response:", res);
      console.log("Team Leaves Data:", res.data);
      
      const data = Array.isArray(res.data) ? res.data : [];
      
      // Log the structure of the first item to see available fields
      if (data.length > 0) {
        console.log("First team leave item structure:", data[0]);
  ...
  renderTeamLeaves = () => {
    ...
    // Filter out leaves with null dates if needed
    const validTeamLeaves = sortedTeamLeaves.filter(leave => 
      leave.startDate && leave.endDate && Array.isArray(leave.startDate)
    );
      ...
      <div className="section-container">
        ...
        {sortedTeamLeaves.length === 0 ? (
          ...
          <div className="table-container">
              ...
              <tbody>
                {sortedTeamLeaves.map(leave => {
                  ...
                  const days = leave.startDate && leave.endDate ? 
                    this.calculateDays(leave.startDate, leave.endDate) : 0;
                  
                  // Check if dates are valid for display
                  ...
                  return (
                    <tr key={leave.recId}>
                      ...
                      <td className={fromDate === '-' ? 'text-muted' : ''}>
                        {fromDate}
                        {fromDate === '-' && <small className="block text-muted">(Not set)</small>}
                      </td>
                      <td className={toDate === '-' ? 'text-muted' : ''}>
                        {toDate}
                        {toDate === '-' && <small className="block text-muted">(Not set)</small>}
                      </td>
            ...
            {/* Show warning if some leaves have missing data */}
            {sortedTeamLeaves.length > validTeamLeaves.length && (
              <div className="data-warning">
                <p>⚠️ Some leave requests have incomplete date information</p>
              </div>
            )}
          </div>
        )}
