...
const resolveUserId = (employee) => {
  // 1. Try all explicit numeric ID fields first
  const candidates = [
    employee.userId, employee.id, employee.empId,
    employee.employeeId, employee.user_id
  ];
  ...
  // 2. Fallback: extract numeric suffix from username (e.g. "VPPL008" → 8)
  //    Only used when backend search DTO does not return userId.
  //    NOTE: This assumes username digits match the DB primary key — fix backend to return userId properly.
