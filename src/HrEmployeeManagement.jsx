...
const prevStep = () => {
  ...
  // 🔥 Skip Manager step (5) in EDIT mode
  if (mode === "edit" && prev === 5) {
    prev = 4;
  }
...
// ✅ ADD THIS (if you don't already fetch these)
// const existingAadhaars = [];
...
// ✅ KEEP ONLY THIS FUNCTION (DO NOT put validation outside)
const validateCurrentStep = () => {
  // clear any old errors from previous validations
  // setErrors({});

  // don't advance if user has already triggered a field-level error
...
if (hasErrors) {
  ...
  // clear any toast message; field errors are shown inline
  setMessage("");
  return true;
};
  ...
  const loadSelectedEmployee = async () => {
    ...
    setForm({
      personalDetailsDTO: {
        ...
        gender:
  selectedEmployee.personalDetailsDTO?.gender
    ...
    : "",
      ...
      maritalStatus:
  selectedEmployee.personalDetailsDTO?.maritalStatus
...
const handleFileChange = async (docKey, file) => {
  ...
  // ✅ Optional: allow only PDF/JPG/PNG (based on your config)
  const allowedTypes = [
    "application/pdf",
    "image/jpeg",
    "image/jpg",
  ...
  try {
    ...
    if (mode === "create") {
      ...
      const payload = {
        ...
        empMgrDto: {
          mgrId: form.empMgrDto.mgrId
            ? Number(form.empMgrDto.mgrId)
            : null
        },
      };
    ...
    // ========================
      ...
      const payload = {
        ...
        bankDetailsDTO: form.bankDetailsDTO,
        employeeStatutoryDetailsDTO:
          form.employeeStatutoryDetailsDTO,
        empMgrDto: {
          mgrId: form.empMgrDto.mgrId
            ? Number(form.empMgrDto.mgrId)
            : null,
      ...
      await axios.put(
        `/api/dept/hr/employee/edit?userId=${userId}`,
        payload,
        {
          withCredentials: true,
  ...
  //       setMessage("❌ Bad request – please check all required fields");
  //     } else if (status === 401) {
  //       setMessage("❌ Unauthorized – you may need to log in again");
  //     } else {
  //       setMessage(`❌ ${err.message}`);
  //     }
  //   } finally {
...
/>
              ...
              <div className="input-group">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (val && message === "❌ Please select gender") setMessage("");
  ...
  onChange={(e) => {
    ...
    if (val.length <= 5) {
      ...
      // First 5 letters + next digits
      val =
        val.slice(0, 5).replace(/[^A-Z]/g, "") +
        val.slice(5).replace(/[^0-9]/g, "");
    } else {
      // Final pattern: 5 letters + 4 digits + 1 letter
      val =
        val.slice(0, 5).replace(/[^A-Z]/g, "") +
...
/>
              ...
              <div className="input-group">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (val && message === "❌ Please select department") setMessage("");
              ...
              <div className="input-group">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (val && message === "❌ Please select designation") setMessage("");
              ...
              <div className="input-group">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (val && message === "❌ Please select date of joining") setMessage("");
                    if (val) setErrors(prev => ({ ...prev, dateOfJoining: "" }));
                  }}
                  className="modern-input"
                />
    ...
    case 5:
  if (mode === "edit") {
    return null; // 🔥 completely hide in edit
  }
        ...
        <div className="manager-search">
          <div className="search-input-wrapper">
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") handleManagerSearch();
              }}
              className="modern-input"
            />
            <button
        ...
        {showManagerSearch && managerSearchResults.length > 0 && (
          <div className="manager-results">
            {managerSearchResults.map((mgr, idx) => (
              <div
                key={idx}
                className="manager-result-item"
                onClick={() => selectManager(mgr)}
              >
                <h5>
                  {mgr.personalDetailsDTO?.firstName} {mgr.personalDetailsDTO?.lastName}
                </h5>
        ...
        {selectedManager && (
          <div className="selected-manager-card">
            <div className="manager-info">
              <h4>
                {selectedManager.personalDetailsDTO?.firstName} {selectedManager.personalDetailsDTO?.lastName}
              </h4>
